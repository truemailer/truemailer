# main.py - Truemailer final release
from fastapi import FastAPI, HTTPException, Request, Form
from fastapi.responses import JSONResponse, FileResponse
from fastapi.middleware.cors import CORSMiddleware
import re, os, time, threading, json, requests, dns.resolver
...
if os.path.exists(CLIENTS_FILE):
    ...
    CLIENTS = {"demo": {"key":"demo_key_123","name":"Demo","limit": CONFIG.get("rate_limit_per_day",100), "usage": {}}}
...
@app.post('/verify')
def verify_post(email: str = Form(...), api_key: str = Form(None)):
    ...
    if domain in CONFIG.get("trusted_providers", []):
        mx = has_mx(domain)
        return JSONResponse({"email": email, "valid": True, "reason": "trusted_provider", "mx": mx, "provider": domain})
    if domain in BLOCKSET:
        return JSONResponse({"email": email, "valid": False, "reason": "disposable_domain", "disposable": True})
    mx = has_mx(domain)
    if not mx:
        return JSONResponse({"email": email, "valid": False, "reason": "no_mx", "mx": False})
    return JSONResponse({"email": email, "valid": True, "reason": "valid", "mx": True, "provider": domain})

@app.post('/update-now')
def update_now(trigger_key: str = Form(None)):
    # optional trigger protection; if provided, must be a client key
    n = refresh_blockset()
